...
public:
	...
	int calcInputbyGradientMethod(Eigen::Matrix<double, INPUT_DOF, 1>& input, const Eigen::Matrix<double, STATE_DIM, 1>& cur_x, double T, double dt, double* BodyValues);//Calculate the input by gradient method
	void calcGradient(Eigen::VectorXd& F, Eigen::VectorXd& x, const Eigen::VectorXd& cur_x, const Eigen::VectorXd& cur_u, const int N, const double dt, double*BodyValues);//Calculate the gradient vector
...
private:
	...
	double calcObjective(const Eigen::VectorXd& x_seq, const Eigen::VectorXd& u_seq, const int N, double* BodyValues);                          //Calculate the total cost function
	...
	double calcHamiltonian(const Eigen::Matrix<double, STATE_DIM, 1>& cur_x, const Eigen::Matrix<double, INPUT_DOF, 1>& cur_u, const Eigen::Matrix<double, STATE_DIM, 1>& cur_lmd, double* BodyValues, const int N);//Calculate the Hamiltonian function
